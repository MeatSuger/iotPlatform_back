...
// @Success      200   {object}  common.ApiResponse{data=entity.User}
...
func (ctl *UserController) Update(c *gin.Context) {
	...
	if currentRole != entity.RoleSuperAdmin && currentRole != entity.RoleAdmin {
		req.ID = currentUID
	} else if req.ID == 0 {
		req.ID = currentUID
	}
...
func (ctl *UserController) GetProfile(c *gin.Context) {
	...
	// 普通用户只能查看自己的信息
	if currentRole != entity.RoleSuperAdmin && currentRole != entity.RoleAdmin && uint(id) != currentUID {
