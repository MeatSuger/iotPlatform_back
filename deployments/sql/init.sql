...
-- ============================================
-- 7. 设备执行器定义表 (iot_device_actuator)
--    (device_id, actuator_id) 唯一，→ iot_device(device_id) 级联删除
--    driver: led / servo / speaker（固件 periph 驱动名）
--    params: 驱动参数 JSON 文本（gpio/count/脉宽范围等，固件 probe 约定）
-- ============================================
CREATE TABLE IF NOT EXISTS iot_device_actuator (
    id          BIGINT       GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    device_id   VARCHAR(50),
