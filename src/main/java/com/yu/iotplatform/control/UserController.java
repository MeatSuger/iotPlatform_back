...
public class UserController {
	...
	public ApiResponse<String> register(@RequestBody User user) {
		if (user.getAccount() == null || user.getAccount().isBlank() || user.getPasswd() == null || user.getPasswd().isBlank()) {
			return ApiResponse.fail(HttpStatus.BAD_REQUEST.value(), "账号和密码不能为空");
		}

		User existUser = userService.getOne(new LambdaQueryWrapper<User>().eq(User::getAccount, user.getAccount()));
		if (existUser != null) {
			return ApiResponse.fail(HttpStatus.BAD_REQUEST.value(), "账号已存在");
		}
		user.setStatus(USER_STATUS_ACTIVE);
		return userService.save(user) ? ApiResponse.success("注册成功", null) : ApiResponse.fail(HttpStatus.BAD_REQUEST.value(), "注册失败");
	}
	...
	public ApiResponse<Page<User>> findPage(@RequestParam(defaultValue = "0") Integer pageNum, @RequestParam(defaultValue = "10") Integer pageSize, @RequestParam(defaultValue = "") String name) {
